...
// Finished-goods records carry no formula/cost data, so operators get full
// view + status-transition access — see root CLAUDE.md's RBAC notes. Backfill
// (a historical-data-repair tool) stays superadmin-only.
...
export async function finishedGoodsRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { batchNumber: string } }>('/:batchNumber', { preHandler: operatorPreHandler }, async (request, reply) => {
  ...
  app.patch<{ Params: { id: string } }>('/:id/status', { preHandler: operatorPreHandler }, async (request, reply) => {
