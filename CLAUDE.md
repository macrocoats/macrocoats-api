...
| `production-planning` | GET /production-planning/overview |
...
- **`GET /sales-analytics`** returns `monthlyTrend`/`quarterlyTrend`/`yearlyTrend` (12 months / 8 quarters / 5 years, zero-filled via `generate_series`, always trailing from today regardless of `range` — deliberately not window-scoped), plus top-10 `salesByCustomer`/`salesByProduct`/`salesByRegion` breakdowns, which **are** scoped to the resolved `range`/`from`/`to` window (via `resolveWindow()`, same as `executive-kpis`) — a report for an empty window shows empty breakdown tables, not an all-time leaderboard. `salesByRegion` is a `companies.state` proxy — there is no dedicated region field in the schema yet.
...
Read-only aggregation over open Customer PO items — "what needs to be manufactured, and what raw materials does that require" — nothing is stored, everything is derived on request. Single route, `GET /production-planning/overview?days=N` (default 7, max 90), superadmin-only.

- **Open items**: `customer_purchase_order_items` joined to non-cancelled/non-completed, non-soft-deleted orders, `remainingQty = quantityOrdered - SUM(customer_purchase_order_batches.linked_quantity)` per item (item-level, not the order-level aggregate `listOrders()` computes). Included if `expected_delivery_date` falls within the window **or is null** — a null date is surfaced as `unscheduled: true` on the item rather than excluded, since "needs a date" is itself an actionable planning gap, not a reason to hide the item.
- **Required materials come from two different shapes depending on whether the item has a `variantId`**: with a variant, `formulation_variant_components` (`materialName`/`percentage`); without one, the product's base formula document (`product_documents` where `docType='formula'`, reading `body.composition`, which uses `name`/`percentWV` — a different field-name shape, see `pages/Products/formula/LiquidFormula.jsx`'s typedef in the frontend). An item whose base-formula composition can't be parsed sets `materialsResolved: false` on that item rather than silently contributing a wrong number to the materials total; the response's `unresolvedItemCount` surfaces how many.
- **A `null` percentage (the water/balance row) never contributes to the materials-required aggregation** — same convention as the frontend's cost calculators: it isn't a distinct raw material to purchase.
- **Materials are aggregated across all open items by `normalizeMaterialName`** (imported from `cost-intelligence.service.ts`, not re-implemented a fourth time — see that module's own note on this being duplicated 3x already; reuse this export rather than adding a 4th copy) and cross-referenced against current `inventory_items.stockQty` for a per-material `shortfall`/`sufficient` flag. This is a **live current-stock check, not a time-phased MRP plan** — it doesn't account for stock already reserved by other in-window items being consumed in sequence; it's "do you have enough today for everything due in this window, summed."
...
- **Refresh token** (7d): JWT whose payload carries `sub` (user id), `jti` (UUID matching a row in `refresh_tokens`), and `sec` (a short opaque random secret). The JWT itself is what's handed to the client — the JWT string is never stored; only a `bcrypt` hash of `sec` is (bcrypt silently truncates inputs over 72 bytes, so the much-longer JWT string is deliberately never what gets hashed — only the short `sec` claim is). Rotation verifies the RS256 signature, revokes the old row by `jti`, bcrypt-verifies `sec` against the stored hash as a second factor, and calls `issueTokens()` to produce a new pair.
...
**Fixed — refresh rotation previously did not work (web or mobile).** `issueTokens()` used to sign a JWT via `signRefreshToken()` but discard it, returning a raw random string as the client-facing refresh token instead; `rotateRefreshToken()` then called `verifyRefreshToken()` (a JWT verify) on whatever the client sent back, which always failed since the client never received a JWT. Fixed by generating the `refresh_tokens` row id (`jti`) client-side via `randomUUID()` before insert, so the JWT (now embedding `sub`/`jti`/`sec`) can be signed and returned in the same pass — see `issueTokens()` for the full flow. `tests/integration/auth.test.ts` now covers a real rotation (new cookies, new access token works, old refresh token rejected on reuse), the mobile refresh flow, and logout revocation.

### Mobile (Bearer-token) auth

`POST /auth/mobile/login`, `POST /auth/mobile/refresh` (`{ refreshToken }` body), `POST /auth/mobile/logout` (`{ refreshToken }` body) in `auth.routes.ts` return tokens in the JSON body (`{ user, accessToken, refreshToken }`) instead of setting cookies, for native clients with no shared cookie jar. They reuse the exact same service functions as the cookie-based web routes (`loginWithCredentials`, `rotateRefreshToken`, `revokeRefreshToken`) — no service-layer differences, only how the route responds. The web routes are untouched; cookie-based auth remains the browser contract.

### RBAC

Three roles: `superadmin`, `company`, and `operator`.
...
- `operator` ("Factory Operator"): a narrow, curated grant — view access to Batches (formula/cost stripped, see below), Finished Goods (full, incl. status transitions), Dispatch (full, incl. creation), and Production Planning (already formula/cost-free by design). No pricing, no formula composition, no company/customer/staff/vendor management, no batch creation, no dispatch void. Not tied to a `companyId` — `allowedProducts` stays `null` for this role, same as superadmin, since batches/dispatch/finished-goods are cross-company operational data, not company-scoped.

`allowedProducts` is loaded from `company_product_access` at login and embedded in the JWT — not re-queried per request.

**Adding an `operator`-accessible route**: use `requireRole('superadmin', 'operator')` (`middleware/requireRole.ts`) instead of swapping `requireSuperAdmin` module-wide — grant per-route, not per-module. Most existing routes should stay superadmin-only untouched. `role` has no DB-level CHECK constraint on `users.role` (confirmed via `\d users` before adding this role) — Drizzle's `enum` there is TypeScript-only narrowing, so adding a new role value needs no migration.

**Batches carry formula/cost data that `company`-role sanitization never had to touch** (`toBatchResponse()` in `batches.service.ts` returns `formulationSnapshot` and `costSummary` directly, unlike `products.service.ts::getDocument()`'s role-aware TDS/MSDS sanitization). `listBatches`/`getBatchByNumber` accept an optional `role` param (`'superadmin' | 'operator'`, default `'superadmin'`) and pick `toBatchResponse` vs. `toOperatorBatchResponse` — the latter strips `formulationSnapshot`/`costSummary` only; `labelSnapshot` and `coaSnapshot` are kept (neither is formula composition or pricing). `batches.routes.ts`'s GET handlers resolve `role` from `request.authUser.role` and pass it through — every other batches route (create/coa/payment/delete) stays superadmin-only, unchanged.
